...
$sql = stream_get_contents($backup); fclose($backup);
...
foreach (array_keys(sbcis_export_queries()) as $table) {
    ...
    $rows = 0; while (fgetcsv($stream, 0, ',', '"', '') !== false) $rows++;
...
foreach (explode("\n", $sql) as $line) if (strpos($line, 'INSERT INTO `') === 0) $db->exec($line);
foreach ($original as $table => $rows) verify($rows === $db->query('SELECT * FROM `' . $table . '` ORDER BY 1')->fetchAll(), 'Exact backup round trip: ' . $table);
...
$difficult = "Quotes '\", slash \\, newline\nUnicode: " . hex2bin('c3b1'); $stmt->execute([$difficult]);
$stream = sbcis_prepare_export($db, 'backup'); $fixtureSql = stream_get_contents($stream); fclose($stream);
$db->exec('DELETE FROM municipalities WHERE municipality_id = 4294967294');
foreach (explode("\n", $fixtureSql) as $line) {
    if (strpos($line, 'INSERT INTO `municipalities`') === 0 && strpos($line, bin2hex($difficult)) !== false) $db->exec($line);
}
...
try { sbcis_prepare_export($db, 'admins'); throw new RuntimeException('Unexpected export'); } catch (InvalidArgumentException $e) { verify(true, 'Reject unsupported export'); }
