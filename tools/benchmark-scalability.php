...
if (!in_array($count, [100, 1000, 5000, 10000, 50000], true)) {
    fwrite(STDERR, "Usage: php tools/benchmark-scalability.php 100|1000|5000|10000|50000\n");
    exit(2);
}
...
$input = ['eligible_count'=>$count];
$prepareMs = null;
$surface = null;
$interpolationMs = null;
if ($count <= Config\InterpolationConfig::MAX_OBSERVATIONS) {
...
$db->exec('CREATE TEMPORARY TABLE boreholes (borehole_id INT UNSIGNED PRIMARY KEY, borehole_code VARCHAR(50) NOT NULL UNIQUE, municipality_id INT UNSIGNED NULL, barangay_id INT UNSIGNED NULL, borehole_depth_m DECIMAL(8,2) NOT NULL, latitude DECIMAL(10,7) NOT NULL, longitude DECIMAL(10,7) NOT NULL, elevation_m DECIMAL(8,2) NULL, created_at TIMESTAMP NULL, updated_at TIMESTAMP NULL, lock_version INT UNSIGNED NOT NULL DEFAULT 1, archived_at DATETIME NULL, archived_by INT UNSIGNED NULL, INDEX(created_at,borehole_id), INDEX(latitude,longitude))');
...
$start = microtime(true);
$mapRows = sbcis_fetch_map_boreholes_window($db, [124.7, 10.0, 125.4, 10.8], 2001);
...
echo json_encode([
    ...
    'surface_cells'=>$surface === null ? null : count($surface['surface']['features']),
    'interpolation_note'=>$surface === null ? 'Skipped above the controlled 10000-observation worker limit.' : null,
    'pagination_query_ms'=>$paginationMs, 'page_rows'=>count($page['rows']),
    'map_query_ms'=>$mapQueryMs, 'map_rows'=>count($mapRows),
    'map_truncated'=>count($mapRows) > 2000, 'map_payload_bytes'=>$mapJsonBytes,
