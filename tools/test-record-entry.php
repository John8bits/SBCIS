...
foreach (['municipalities', 'barangays', 'boreholes', 'soil_layers'] as $table) {
    ...
    foreach ($db->query('SHOW COLUMNS FROM ' . $table)->fetchAll() as $column) {
        ...
        if ($column['Default'] !== null)
            $definition .= ' DEFAULT ' . (stripos($column['Default'], 'current_timestamp') === 0 ? 'CURRENT_TIMESTAMP' : $db->quote($column['Default']));
        $columns[] = $definition;
    }
...
if ((int) $db->query('SELECT COUNT(*) FROM boreholes')->fetchColumn() !== 1 || (int) $db->query('SELECT COUNT(*) FROM soil_layers')->fetchColumn() !== 2)
    throw new RuntimeException('Record and layers were not saved together.');
$checks++;
$rows = $db->query('SELECT * FROM soil_layers ORDER BY layer_number')->fetchAll();
if ((int) $rows[0]['spt_n_value'] !== 0 || $rows[1]['spt_n_value'] !== null)
    throw new RuntimeException('Zero and missing SPT values were not preserved.');
$checks++;
$loaded = sbcis_fetch_geotechnical_record($db, $id);
if (!$loaded || count($loaded['layers']) !== 2 || $loaded['borehole_code'] !== 'TEST-ENTRY')
    throw new RuntimeException('Edit record could not be loaded.');
$checks++;
$updated = $input;
$updated['borehole_code'] = 'TEST-UPDATED';
$updated['borehole_depth_m'] = '12';
$updated['soil_type'] = ['Updated clay'];
$updated['depth_from_m'] = ['0'];
...
try {
    sbcis_update_geotechnical_record($db, $id, $invalidUpdate);
    throw new RuntimeException('Invalid update accepted.');
} catch (InvalidArgumentException $e) {
    $checks++;
}
if (sbcis_fetch_geotechnical_record($db, $id)['layers'][0]['soil_type'] !== 'Updated clay')
    throw new RuntimeException('Failed update changed saved data.');
$checks++;
try {
    sbcis_create_geotechnical_record($db, $updated);
    throw new RuntimeException('Duplicate accepted.');
} catch (PDOException $e) {
    $checks++;
}
if (sbcis_fetch_geotechnical_record($db, $id) === null)
    throw new RuntimeException('Duplicate changed record.');
$checks++;
$municipalityId = sbcis_find_or_create_municipality($db, 'City of Test municipality');
if ((int) $db->query('SELECT COUNT(*) FROM municipalities')->fetchColumn() !== 1)
    throw new RuntimeException('Directory city alias created a duplicate.');
$checks++;
...
$checks++;
if (sbcis_fetch_geotechnical_record($db, $id) !== null || (int) $db->query('SELECT COUNT(*) FROM soil_layers')->fetchColumn() !== 0)
    throw new RuntimeException('Delete did not cascade to layers.');
$checks++;
if (sbcis_delete_geotechnical_record($db, $id))
    throw new RuntimeException('Missing record reported deleted.');
$checks++;
